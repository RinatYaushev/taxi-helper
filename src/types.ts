...
export interface Trip {
   ...
   *  («7 хв»). Разом із `pickup_km` розділяє «далеко» і «пробка», а також
   *  калібрує темп руху напряму, без регресії. */
  pickup_min?: number;
  from: string;
  to: string;
  zone: Zone;
 ...
 * Записується вручну: це єдиний спосіб дізнатися **ціну відмови** (скільки
 * коштує звуження фільтра), бо відхилені фільтром замовлення нам не видно.
 */
export interface Idle {
  /** Коли простій почався, "DD.MM HH:MM". */
  datetime: string;
  /** Скільки хвилин простояв без замовлення. */
  minutes: number;
  /** Де саме стояв — місто чи глухий кут. Закриває питання «чи тупик
   *  насправді дорожчий», яке на самих поїздках не вирішується. */
  zone?: Zone;
  /** Який радіус подачі стояв у фільтрі під час простою, км. Дозволяє
   *  виміряти, скільки простою купує вузький радіус. */
  radius_km?: number;
  /** Довільна нотатка (напр. «дощ», «після зміни ТЦ»). */
  note?: string;
}
...
export interface Data {
  settings: Settings;
  trips: Trip[];
  /** Ручні записи простоїв. Опційно — старі дані їх не мають. */
  idles?: Idle[];
}
