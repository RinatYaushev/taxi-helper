// Панель «Фільтри Автопілота»: 2 постійні слоти — і більше нічого.
//
...
/**
 * Що робити з глухим кутом, коли фільтр не допоможе.
 *
 * ⚠️ Це навмисно НЕ фільтр і не третій слот. `zone` визначається за
 * призначенням поїздки, а форма Uklon дає лише радіус/сектор подачі від
 * поточної позиції — поля «я зараз застряг у тупику» там просто немає.
 * Тому будь-яка спроба «налаштувати слот під втечу з глухого кута»
 * зводиться до звичайного дешевого-і-широкого фільтра, який однаково діє
 * всюди, а не тільки в тупику. Чесніше показати це прямо, ніж видавати
 * загальний слот за адресне рішення.
 */
function deadEndEscapeNote(): string {
  return `
    <div class="fx-note fx-note-top">
      <div class="fx-note-t">🧭 Як вибиратись із глухого кута</div>
      <p>Фільтр тут об'єктивно не допоможе: <b>зона визначається за
        призначенням</b> поїздки, а не за тим, де ти зараз стоїш. У формі
        Uklon немає поля «я застряг» — лише радіус чи сектор подачі від
        поточної позиції. Це завжди буде ручне рішення в моменті, а не
        налаштування, яке можна забути.</p>
      <ol class="fx-howto-list">
        <li><b>Ефір.</b> Перевір стрічку вручну — вона не прив'язана до
          фільтра, і саме туди Uklon підкидає «додаткові» замовлення під час
          простою.</li>
        <li><b>Ланцюжком.</b> Не чекай одну поїздку, що одразу вивезе в
          місто — бери короткі проміжні в потрібному напрямку одну за одною.</li>
        <li><b>Під'їдь ближче.</b> Фізично скоротити відстань до жвавого
          району дешевше, ніж розширювати радіус подачі: те саме замовлення
          стає доступним без зміни фільтра.</li>
      </ol>
      <p class="fx-note-sub">Занеси випадок у <b>журнал простоїв</b>
        (<code>data.idles</code>: <code>minutes</code>, <code>zone</code>,
        <code>radius_km</code>) — це єдиний спосіб колись виміряти, який із
        трьох варіантів справді працює найкраще, а не гадати по кількох
        випадках.</p>
    </div>`;
}
...
function filtersCommon(rows: Row[], s: Settings): string {
  ...
  return `
    <div class="fx-rules">
      ...
      <div class="fx-rule">
        <div class="fx-rule-t">2 активні фільтри, об'єднані за АБО</div>
    ...
    <div class="fx-note fx-note-top">
      ...
      обов'язково вписувати у фільтр.
      <div class="fx-tag-row"><b>🏙 Живі райони</b>: ${tags(s.live_areas, "tag-live")}</div>
      <div class="fx-tag-row"><b>🚧 Глухі кути</b>: ${tags(s.dead_end_areas, "tag-dead")}</div>
      <div class="fx-tag-row"><b>🚫 Дальняк</b>: ${tags(s.long_haul_areas ?? [], "tag-haul")}</div>
    </div>`;
}
  ...
  return `
    <div class="panel fx-panel fx-hero">
      <div class="fx-hero-head">
        <h3>🎯 Фільтри Автопілота — 2 постійні слоти</h3>
      ...
      <div class="fx-approach">
        ...
        <p class="fx-approach-p">
          Два слоти вписуються у два активні фільтри Uklon і <b>лишаються ввімкненими
        ...
        <div class="fx-howto">
          ...
          <ol class="fx-howto-list">
            <li>Створи в Автопілоті <b>два фільтри</b> й перепиши поля з карток
            ...
            <li>Увімкни <b>обидва одночасно</b> і більше не чіпай.</li>
      ...
      <div class="grid2 fx-grid">${cards}</div>
      ${sectorsNote(rows, s, slots)}
      ${deadEndEscapeNote()}

      <div class="fx-switch-title">Як читати пару і який сетап обрати</div>
      <div class="fx-note fx-note-top">
        Слоти — це <b>драбина за подачею</b>, а не два різні «настрої».
        Кожен наступний <b>строгіший за ціною, але дозволяє їхати далі</b> по клієнта.
        Поточна розкладка: <b>${esc(ladder)}</b>.
        ${withPickup < 20
          ? `<br><br>⚠️ Рядок <b>«тільки цей слот»</b> для другого показує мало —
