...
function modeFields(m: Mode): string {
  ...
  // (короткі вимагають вищої ₴/км). Плоский важіль наближає її лише частково.
  const manual: string[] = [];
    ...
    <div class="fx-tabpane" id="pane-slots" role="tabpanel" aria-labelledby="tab-slots">
      <div class="fx-approach">
        ...
        <p class="fx-approach-p">
          ...
          Саме це робить їх невкладеними — за АБО фільтр із тією ж ціною і більшим
          «Км у мінімалці» був би строгою <b>підмножиною</b> сусіда й не додавав би
          нічого.
        </p>
        <div class="fx-howto">
          ...
          <ol class="fx-howto-list">
            ...
            <li>У блоці «Тариф» обовʼязково обери вкладку <b>«Складний»</b>.
              Полів «Км у мінімалці» та окремої ціни для передмістя у «Простому»
              <b>немає</b>, і якщо лишити активним його, ці значення просто
              <b>ігноруються</b> — мовчки, без попередження. Заповнити можна обидві
              вкладки, але працює <b>лише обрана</b>.</li>
            <li>Постав кожному режим <b>«Цикл»</b>, а не «Авто». В «Авто» фільтр
              шукає замовлення <b>один раз</b> і вимикається одразу після того, як
              ти приймеш наступне або відмовишся — тоді «увімкнув і забув»
              не працює. «Цикл» тримає пошук безперервно.</li>
            <li>Увімкни <b>всі три одночасно</b> і більше не чіпай.</li>
            <li>Пам'ятай: якщо пропустити або відхилити <b>обов'язкове</b>
              замовлення, автофільтри <b>вимикаються</b>. Після такого перевір,
              чи вони ще ввімкнені.</li>
      ...
      <div class="fx-note fx-note-top">
        ...
        дешеве беремо лише впритул, за дорогим не шкода й проїхатись.
        Поточна розкладка: <b>${esc(ladder)}</b>.
        <br><br>
        ⚠️ Рядок <b>«тільки цей слот»</b> зараз показує <b>0</b> для другого й третього —
        і це <b>очікувано</b>, а не ознака, що вони зайві. Бектест не бачить подачі:
        поле <code>pickup_km</code> заповнене в <b>0 із ${rows.length}</b> поїздок, тож
        перевірка радіуса просто пропускається, і слоти виглядають як вкладені за ціною.
        Щойно зʼявиться 20–30 замірів подачі, перший слот почне відсікати далеких
        клієнтів, а другий і третій — їх підбирати. Тоді цей рядок стане осмисленим.
      </div>
    ...
    <div class="fx-rules">
      <div class="fx-rule">
        ...
        <div class="fx-rule-note">Тому підняття <b>«Км у мінімалці»</b> б'є саме по
          ...
          <br><br>
          ⚠️ Це <b>інференс</b>, а не цитата. Офіційний опис описує поле двома
          <b>несумісними</b> прикладами: «Км у мінімалці» пояснено як
          <i>адитивну</i> формулу (перші K км коштують мінімалку, далі доплата за км),
          а «Мін. ціна км» — як окрему умову на <b>всю</b> дистанцію. Адитивний
          варіант <b>слабший</b>: на 10 км він вимагає 220 ₴ там, де наш — 280 ₴.
          Різниця на історії — <b>12 поїздок</b>, усі в глухі кути, 220 ₴/год.
          Перевіряється однією зміною: якщо при 28 ₴/км і K=5 прийде замовлення
          ~240 ₴ на 10 км — правило адитивне, і поріг для передмістя треба піднімати.
        </div>
      </div>
      <div class="fx-rule">
