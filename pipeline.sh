...
# зіставляючи скрін із поїздкою за відбитком «сума + дистанція» (НЕ за датою —
# свіжі замовлення застосунок показує без неї). Самі поїздки додає
# `npm run parse --write` із /tmp/ocr.txt — див. AGENTS.md.
